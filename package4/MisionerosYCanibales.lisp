...
(defun  valid-operator? 
	...
	(let* (
    	...
    	(and
			(>=  misioneros  (first (second op))) ;;el número de misioneros a mover, primer elemento de la lista-operador
...
(defun  valid-state? 
    ...
    el estado tiene estructura:  [(<m0><c0><b0>) (<m1><c1><b1>)]"
    (let (
		(m0  (first (first estado))) ;;el estado tiene estructura ((<m0><c0><b0>) (<m1><c1><b1>)) ...
		(c0  (second (first estado)))
		(m1  (first (second estado)))
		(c1  (second (second estado))))
		(and  
			(or  (>=  m0 c0) (zerop m0)) ;;mayor o igual número de misioneros que caníbales, excepto si hay 0 misioneros
    	    (or  (>=  m1 c1) (zerop m1)))
		)  
	)
    ...
    (let* (
		...
		(operador (first op))) ;; este operador es la etiqueta humana del operador...
	 	(case operador 
	    	(:Dos-Misioneros 
				(if 
					(= orilla-barca 0)  ;; restar elementos de la orilla con la barca y sumarlos en la otra orilla...
	                (list  
						(list (- m0 2) c0 (flip b0))   
						(list (+ m1 2) c1 (flip b1)))
					(list  
						(list (+ m0 2) c0 (flip b0))  
						(list (- m1 2) c1 (flip b1)))
					)
	    	...
	    	(:Un-Misionero   
				...
				(= orilla-barca 0)  
	                (list  
						(list (- m0 1) c0 (flip b0))   
						(list (+ m1 1) c1 (flip b1)))
					(list  
						(list (+ m0 1) c0 (flip b0))  
						(list (- m1 1) c1 (flip b1)))
					)
	    	...
	    	(:Misionero-y-Caníbal  
	            (if 
					(= orilla-barca 0)  
					(list  
						(list (- m0 1) (- c0 1) (flip b0))   
						(list (+ m1 1) (+ c1 1) (flip b1)))
					(list  
						(list (+ m0 1) (+ c0 1) (flip b0))  
						(list (- m1 1) (- c1 1) (flip b1)))
					)
	    	...
	    	(:Un-Caníbal 
				(if 
					(= orilla-barca 0)  
			   		(list  
						(list m0 (- c0 1) (flip b0))   
						(list m1 (+ c1 1) (flip b1)))
			   		(list  
					   (list m0 (+ c0 1) (flip b0))
					   (list m1 (- c1 1) (flip b1)))
					)
	    	...
	    	(:Dos-Caníbales
				(if 
					(= rilla-barca 0)  
	            	(list  
						(list m0  (- c0 2) (flip b0))
						(list m1 (+ c1 2) (flip b1)))
					(list  
						(list m0 (+ c0 2) (flip b0))
						(list m1 (- c1 2) (flip b1)))
					)
...
(defun remember-state? 
    ...
    (cond 
		((null  lista-memoria)  
			NIL)
	    ((equal  estado  (second (first  lista-memoria)))  
			T) ;;el estado es igual al que se encuentra en el nodo?
		(T  
			(remember-state?  estado  (rest  lista-memoria))))
	)
...
(defun  filter-memories 
    ...
    (cond 
		...
		(T  (cons  (first lista-estados-y-ops) 
			(filter-memories  (rest  lista-estados-y-ops)))) ;; de lo contrario, incluirlo en la respuesta
		) 
	)
...
(defun extract-solution 
    ...
    (labels (
		...
		(cond 
		    ...
		    (T  
				(locate-node  id (rest lista)))))
		)
	  	(let (
			(current (locate-node (first  nodo)  *memory*)))
  	...
  	(let (
		(nodo nil)
      	...
      	(loop until 
			...
			do
			(setq 
				nodo (get-from-open) ;;Extraer el siguiente nodo de la frontera de búsquea
	   		...
	   		(cond 
		        ...
		        (t 
					...
					(format T "Este es el hijo ~d~%" sucesores)
